...
(() => {
  ...
  /* ---------------------------------------------------------------------
     ...
     --------------------------------------------------------------------- */
  const revealEls = document.querySelectorAll('.reveal, .reveal-on-load');
  ...
  rsvpAccept.addEventListener('click', () => {
    ...
    launchConfettiBurst();
    // Hook point: send RSVP "accepted" to your backend / form service here.
  });
  ...
  // Gentle ambient hearts drifting from the thank-you section once it's in view
  const thankYouSection = document.getElementById('thankyou');
  let ambientHeartsStarted = false;
  const thankYouIO = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting && !ambientHeartsStarted) {
        ambientHeartsStarted = true;
        launchFloatingHearts(8);
        launchConfettiBurst();
      }
    });
  }, { threshold: 0.5 });
  if (thankYouSection) thankYouIO.observe(thankYouSection);
